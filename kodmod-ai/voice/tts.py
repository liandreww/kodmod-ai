...
try:
    ...
    # Restricted env / text-only testing — the dir is only needed when a TTS
    # backend actually runs. Don't crash at import time.
    pass
...
async def tts_node(state: KODMODState) -> dict[str, Any]:
    from config.settings import settings  # local import avoids import cycle

    if not settings.TTS_ENABLED:
        # Text-only mode: skip synthesis entirely, keep the graph shape intact.
